...
! IAF
    use if_conv_tend,  only: use_IAF_gci, gci_IAF, use_IAF_wci, wci_IAF,  &
                             use_IAF_wcl, tmp2l_IAF
...
! IAF: not using MG here
            ...
            if (use_IAF_wcl == 1) tmp2l = tmp2l_IAF
          
!           print*,'tmp2l = ',tmp2l
!           print*,'rel(i,k) = ',rel(i,k)
!           print*,'cbarli = ',cbarli
!           print*,'dbarli = ',dbarli
...
! IAF
! the wci calculations done here are for a particular solar band only
!
            if (use_IAF_wci == 1) then
              tmp2i_old = tmp2i
              tmp2i = wci_IAF*tmp2i + (1.-wci_IAF)*.999999_r8
              wci(i,k) = min(tmp2i,.999999_r8)
!             print*,'setting wci equal to wci_IAF'
!             print*,rei(i,k),wci(i,k),tmp2i_old
            else
              wci(i,k) = min(tmp2i,.999999_r8)
            endif
...
!
!  IAF:
!  ebarii is just a parameter
!  Above: tmp3i = fbarii*rei(i,k)
!  This is (8) of Ebert and Curry
!
            gci(i,k) = ebarii + tmp3i

!  IAF
            if (use_IAF_gci == 1) then
              gci(i,k) = gci_IAF 
!             print*,'setting gci equal to gci_IAF'
            endif

            fci(i,k) = gci(i,k)*gci(i,k)
! IAF
!            print*,'i k = ',i,k
!            print*,'gci(i,k) = ',gci(i,k)
!            print*,'fci(i,k) = ',fci(i,k)
!            print*,'------'
...
!
!  IF IAF: deactivated this is to try and get FUS, etc
! if (single_column.and.scm_crm_mode) then 
   ! Following outputs added for CRM
!  print*,'outfld for FUS'
