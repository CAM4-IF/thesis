...
subroutine cldfrc_getparams(rhminl_out, rhminh_out )
!-----------------------------------------------------------------------
...
! CFF
!
    use if_conv_tend,  only: sigmoidal, cloud_fraction_max, ri_scale,  &
                             rl_scale, rl_base, ce_cf,                 &
                             cf_max_ri, cf_max_rl, ce_amp,             &
                             t_ri_half,t_ri_scale, cape_mean_activate, &
                             ri_min,ri_add,                            &
                             cf_ri_min
    use if_conv, only: ifconvection_activated, cape_meann, conv_energy_all, &
                       i_cf_all, cf_tar_all
...
!      clouds and make this one just responsible for relative humidity clouds
    ...
    real(r8), intent(in) :: cldliq(pcols,pver)    ! cloud liq mixing ratio  IAF add
...
!  CFF
!
    real(r8) cf_ri, cf_rl, log_ri, xxx, tt, sigmoid, ce_fact, cee
    real(r8) qq,ri_half
...
!   print*,'entering subroutine cldfrc'
...
!   print*,'entering cloud_faction'
    ...
    !
    !  IAF: I don't think icecldf or liqldf are calculated here. Just zero.
    !
    !
    !==================================================================================
...
!   
!  IAF: This block for convective cloud fractions is sometimes entered. shallowcu, deepcu,
!    and concld always zero, as would be expected.
!   
#ifndef PERGRO
!   print*,'use_shfrc = ',use_shfrc
    do k=1,pver-1
       do i=1,ncol
...
!         print*,'i k = ',i,k
!         print*,'shallowcu(i,k) = ',shallowcu(i,k)
!         print*,'deepcu(i,k) = ',deepcu(i,k)
!         print*,'concld(i,k) = ',concld(i,k)
       end do
    end do
#endif
    !==================================================================================
    ...
    !
    !
    !  IAF: the computation of rhcloud is mostly where cloud_fraction comes from
    !
    !
    !====================================================================
...
ri_half = sigmoidal(temp(i,k),t_ri_half,t_ri_scale,ri_min,ri_add)
if (ifconvection_activated(i) == 1) then
  if (cldice(i,k) > 1.0E-12) then
    if (ri_half < 0.) then
      stop 'ri_half is negative in cloud_fraction.F90'
    endif
!-----------------------------------------------
!  Calculate cf_ri: cloud ice fraction
!-----------------------------------------------
!
...
! print*,'-------------'
! print*,'cldice(i,k) = ',cldice(i,k)
! print*,'ri_scale = ',ri_scale
! print*,'log_ri = ',log_ri
! print*,'log(ri_half) = ',log(ri_half)
! print*,'log(ri_scale) = ',log(ri_scale)
! print*,'tt = ',tt
! print*,'exp(-tt) = ',exp(-tt)
! print*,'cf_ri = ',cf_ri
    endif
!-----------------------------------------------
!  Enforce minimum value in cf_ri
!-----------------------------------------------
   if (cf_ri < cf_ri_min) cf_ri = 0.
!-----------------------------------------------
!  Set cf_ri equal to zero if ri = 0
!-----------------------------------------------
  else
    cf_ri = 0.
  endif
!-----------------------------------------------
!
!  Calculate cf_rl: cloud liquid fraction
!
!-----------------------------------------------
  cee = conv_energy_all(i)
  if (cldliq(i,k) > 1.0E-14) then
    if (k >= pver-4) then
      if (k == pver + 1 - i_cf_all(i)) then
        cf_rl = cf_tar_all(i)
      else
        cf_rl = 0.
      endif
!     print*,'------ i k = ',i,k
!     print*,'i_cf_all(i) = ',i_cf_all(i)
!     print*,'cee = ',cee
!     print*,'cf_rl = ',cf_rl
!     print*,'cldliq(i,k) = ',cldliq(i,k)
!     print*,'--------------'
    else
      ce_fact = ce_amp/(1.+log(1.+(cee/ce_cf)))
      cf_rl = ce_fact*(cldliq(i,k)/rl_scale)
    endif
    if (cf_rl > cf_max_rl(k)) cf_rl = cf_max_rl(k)
!-----------------------------------------------
!  diagnostics
!-----------------------------------------------
! print*,'-------------'
! print*,'cldliq(i,k) = ',cldliq(i,k)
! print*,'rl_scale = ',rl_scale
! print*,'log(rl_scale) = ',log(rl_scale)
! print*,'tt = ',tt
! print*,'exp(-tt) = ',exp(-tt)
! print*,'-------------'

!-----------------------------------------------
!  rl does not exceed minimum: set cloud fraction equal to zero.
!-----------------------------------------------
  else
    cf_rl = 0.0
  endif
!-----------------------------------------------
!  endif for if (ifconvection_activated(i) == 1) 
!-----------------------------------------------
endif
  

          !++ag   This is now designed to apply FOR LIQUID CLOUDS (condensation > RH water)
!         print*,'In column for layer cloudiness'
...
!--------------------------------------------------------------------------------------
!
!  IAF adjustment of low cloud fraction in convective regions
!  CFF
!--------------------------------------------------------------------------------------
         if (ifconvection_activated(i) == 1) then
           rhcloud(i,k) = max(0.0_r8,min(cf_rl + cf_ri,cloud_fraction_max))
         endif

           if (rhcloud(i,k) < 1.0E-3) rhcloud(i,k) = 0.0_r8
...
!--------------------------------------------------------------------------------------
!
!  IAF adjustment of high cloud fraction in convective regions
!  - presumably cldliq mostly zero here.
!  CFF
!--------------------------------------------------------------------------------------
         if (ifconvection_activated(i) == 1) then
           rhcloud(i,k) = max(0.0_r8,min(cf_rl + cf_ri,cloud_fraction_max))
...
!--------------------------------------------------------------------------------------
!
!  IAF adjustment of middle cloud fraction in convective regions
!  CFF
!--------------------------------------------------------------------------------------
         if (ifconvection_activated(i) == 1) then
           rhcloud(i,k) = max(0.0_r8,min(cf_rl + cf_ri,cloud_fraction_max))
...
!
!  IAF: 
!  program goes here but I am not sure why.
!  cloud is redefined below using rhcloud. 
!
             ! For RK microphysics
             cloud(i,k) = rhcloud(i,k)
!            print*,' RK microphysics'
          end if
       end do
    end do 

!   print*,'finished AAA loop'
    !
...
!
!  IAF: cldst usually zero but not always
!
!  print*,'rhcloud(i,k) = ',rhcloud(i,k)
!  print*,'cldst(i,k) = ',cldst(i,k)
!  print*,'concld(i,k) = ',concld(i,k)
!  IAF: cloud defined here
!
...
!
          ...
          !  IAF: concld here is always zero, as expected since ZM conv mass fluxes zero.
          !
          cloud(i,k) = min(cloud(i,k)+concld(i,k), 1.0_r8)
!
!         if (k == pver + 1 - i_cf_all(i)) then
!         if (ifconvection_activated(i) == 1) then
!         if (conv_energy_all(i) < 1.) then
!           print*,'------- in cloud_fraction ----------'
!           print*,'i k = ',i,k
!           print*,'ifconvection_activated(i) = ',ifconvection_activated(i)
!           print*,'clat(i) = ',clat(i)
!           print*,'latitude = ',clat(i)*57.296_r8
!           print*,'conv_energy_all(i) = ',conv_energy_all(i)
!           print*,'cldst(i,k) = ',cldst(i,k)
!           print*,'final cloud(i,k) = ',cloud(i,k)
!           print*,'rhcloud(i,k) = ',rhcloud(i,k)
!           print*,'cf_tar_all(i) = ',cf_tar_all(i)
!           print*,'cf_rl = ',cf_rl
!           print*,'---------------------------------------'
!         endif
!         endif
