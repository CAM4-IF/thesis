...
\begin{table}[H]
\caption {$\overbar{P}$, $\overbar{q}_{bl}$, inferred convective mass flux and $\overbar{\omega}_{500}^{\uparrow}$ response. All quantities indicated here are tropical means (30$^\circ$S-30$^\circ$N). Here, $\overbar{q}_{bl}$ is the mean $\overbar{q}$ of the three lowest model levels to represent the mean boundary layer $\overbar{q}$.} \label{tab:title} 
\begin{center}
...
\caption{Fractional response of $\langle\overbar{\omega}_{500}^{\uparrow}\rangle$ compared to the inferred mass flux fractional response, $\delta\overbar{M}^{`}/\overbar{M}^{`} \equiv \langle\delta\overbar{P}/\overbar{P}\rangle - \langle\delta\overbar{q}_{bl}/\overbar{q}_{bl}\rangle$, in various versions of the CAM4-IF. Model ``1" is the default CAM4 (the numbered model versions in table \ref{tab:overview} correspond to the numbers in this plot). Note that upward omega is not the same as convective mass flux. The dashed line is the linear least-squares fit, with the solid line the line indicating what the relationship would be if there was a  one-to-one relationship between inferred mass flux and ${\overbar{\omega}_{500}^{\uparrow}}$.}
\label{fig:vecchi}
\end{figure}
...
\begin{figure}[H]
\centering
\noindent\includegraphics[width=0.9\linewidth]{../figures/FMASS_ANN-crop.pdf}\hfill
...
\begin{table}[H]
\caption {Fractional response of various Walker circulation strength metrics for the CAM4/CAM4-IF models and for AMIP4K models which data is available. $\delta({(\overbar{\omega}^{*})_{500}^{\uparrow}})/(\overbar{\omega}^{*})_{500}^{\uparrow}$ is the response of the ascending region, 10S-10N/90E-180E 500 hPa, upward zonally anomalous omega. $\frac{\delta{\overbar{P}^{*}}}{\overbar{P}^{*}}$ is the repsonse of the zonally anomalous precipitation for the ascending region. $\frac{\delta(\overbar{Q}_{tot})^{*}}{(\overbar{Q}_{tot})^{*}}$ is the response of the zonally anomalous total diabatic heating intergrated thorough the troposphere for the ascending region. Finally, $\frac{\delta(\overbar{\chi}_{200})^{*}}{(\chi_{200})}$ is the response of the 200 hPa zonally anomalous velocity potential for the ascending region. The CAM4-IF models generally weaken the WC more than the CMIP5 AMIP models (based on 200 hPa $\chi^{*}$). The correlation between the $(\overbar{\omega}^{*})_{500}^{\uparrow}$ and $\overbar{\chi}_{200}^{*}$ measures of WC strength for the CAM4-IF models is 0.86. The correlation between $\frac{\delta(\overbar{Q}_{tot})^{*}}{(\overbar{Q}_{tot})^{*}}$ and $\delta({(\overbar{\omega}^{*})_{500}^{\uparrow}})/(\overbar{\omega}^{*})_{500}^{\uparrow}$ is 0.77, while the correlation between $\frac{\delta(\overbar{\chi}_{200})^{*}}{(\overbar{\chi}_{200})}$ and $\frac{\delta(\overbar{Q}_{tot})^{*}}{(\overbar{Q}_{tot})^{*}}$ is 0.84. The correlation between $\frac{\delta{\overbar{P}^{*}}}{\overbar{P}^{*}}$ and $\delta({(\overbar{\omega}^{*})_{500}^{\uparrow}})/(\overbar{\omega}^{*})_{500}^{\uparrow}$ is 0.66 for the CAM4-IF models and 0.67 for the correlation between $\frac{\delta{\overbar{P}^{*}}}{\overbar{P}^{*}}$ and $\frac{\delta(\overbar{\chi}_{200})^{*}}{(\overbar{\chi}_{200})}$. Finally, the relationship between $\frac{\delta{\overbar{P}^{*}}}{\overbar{P}^{*}}$ and $\frac{\delta(\overbar{\chi}_{200})^{*}}{(\overbar{\chi}_{200})}$ for \textbf{all} models is 0.48. This demonstrates that the total zonally anomalous diabatic heating response and the zonally anomalous precipitation response are moderately correlated with the response of the WC as measured by either $(\overbar{\omega}^{*})_{500}^{\uparrow}$ or $(\overbar{\chi}_{200})^{*}$.} \label{tab:WC} 
\begin{center}
...
\caption{\textbf{a)-c)}: 200 hPa annual mean $\overbar{\chi}^{*}$ response (shaded) with the control run annual mean climatology in contours. Stipling indicates regions where the response is larger in size than the largest (same-signed) response of the CMIP5 AMIP models.\textbf{d)}: The AMIP4K ensemble mean response of $\overbar{\chi}^{*}$. Note the closer fit of the control and response of the AMIP mean to the CAM4-IF. Stippling indicates regions where 10 or more of 12 models agree on the sign of the response.}
\end{figure}
